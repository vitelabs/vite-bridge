...
export default class vitebridge {
  constructor(
    { readyCallback, selfDefinedMethods = builtInMethods } = {
      selfDefinedMethods: builtInMethods
    }
  ) {
        ...
        return new Promise((res, rej) => {
          ...
          const callback = (function(res, rej) {
            return function(responseArgs) {
              const { code = 0, msg = "", data = null } = JSON.parse(
                responseArgs
              );
              //timeout todo
              if (code === 0) {
          ...
          if (!this._ready) {
            console.log(
              `call ${m} when not ready,${JSON.stringify(callHandlerArgs)}`
            );
            this.callHandleCacheQueu.push(callHandlerArgs);
          } else {
            console.log(
              `call ${m} when ready,${JSON.stringify(callHandlerArgs)}`
            );
  ...
  callAnyway(methodName, arg) {
    return new Promise((res, rej) => {
      this._originBridge.callHandler(
        methodName,
        arg,
        (function(res, rej) {
          return function(responseArgs) {
