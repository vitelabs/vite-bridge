...
function insertTpl(name, params) {
  const tpl = document.createElement('div')
  tpl.classList.add('tpl')
  tpl.classList.add(marshalMethodName(name))

  const title = name ? `<h4>${name}</h4>` : '<input>'
  const param = params ? `<textarea class="param" rows="1">${JSON.stringify(params)}</textarea>` : ''
  tpl.innerHTML = `${title}
                  ${param}
                  <textarea class="content"></textarea>
                  ${params? '<button type="button">call</button>': ''}`
...
function attachClickEvent(tpl, clickEvent, autorun) {
  const inputEl = tpl.querySelector('.param');
  const contentEl = tpl.querySelector(".content");

  const run = () => clickEvent(contentEl, inputEl)
  if (autorun) {
    run()
  }
...
for (const method in methods) {
  ...
  attachClickEvent(tpl, (contentEl, inputEl) => {
    let arg = inputEl && inputEl.value.trim();
    try {
