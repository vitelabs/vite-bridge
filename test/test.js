  ...
  const template = `<div class="tpl card ${testKey
    ...
    .replace(".", "-")}">
        ...
        ${
          isSub
            ? ""
            : '<button class="btn btn-secondary" type="button" style="margin:10px auto;">call</button>'
        }
    </div>`;
  $("#app").append(template);
}
...
methods.forEach(m => {
  ...
  attachClickEvent(m, (contentEl, inputEl) => {
    ...
    if (m === "wallet.sendTxByURI") {
      arg = Object.assign({ address }, arg || {});
      if (!arg.address) {
        contentEl.textContent("missing address params");
        return;
      }
    }
